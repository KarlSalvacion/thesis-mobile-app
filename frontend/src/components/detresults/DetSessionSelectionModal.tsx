...
const DetSessionSelectionModal = ({
  visible,
  sessions,
  selectedDetection,
  onClose,
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await fetch(`${API_BASE}/detection/${session[0]}`, {
                method: 'DELETE',
              })
              if (response.ok) {
    ...
    <Modal visible={visible} transparent={true} animationType="slide" onRequestClose={onClose}>
      <View className="flex-1 bg-black/60 justify-end">
        <View className="bg-white rounded-t-2xl p-4 max-h-3/4">
                ...
                <View className="flex-row items-center justify-between">
                  <View className="flex-1">
                    ...
                    <Text className="text-xs text-gray-500">
                      {formatAmPm(session[2])} • {session[3]}
                    </Text>
                  </View>
          ...
          <TouchableOpacity className="mt-3 p-3 bg-gray-100 rounded-lg" onPress={onClose}>
...
export default DetSessionSelectionModal
